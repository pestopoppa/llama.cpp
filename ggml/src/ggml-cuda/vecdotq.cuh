...
#pragma unroll
    for (int i = 0; i < vdr; ++i) {
        const int vhi = vh >> (4 * i);
        int vi0 = (vl[i] >>  0) & 0x0F0F0F0F; // lower 4 qs bits, still need qh as 5th bits
        vi0    |= (vhi <<  4) & 0x00000010; // 0 ->  4
        vi0    |= (vhi << 11) & 0x00001000; // 1 -> 12
        vi0    |= (vhi << 18) & 0x00100000; // 2 -> 20
        vi0    |= (vhi << 25) & 0x10000000; // 3 -> 28
        sumi = ggml_cuda_dp4a(vi0, u[2*i+0], sumi); // SIMD dot product of quantized values

        int vi1 = (vl[i] >>  4) & 0x0F0F0F0F; // upper 4 qs bits, still need qh as 5th bits
        vi1    |= (vhi >> 12) & 0x00000010; // 16 ->  4
        vi1    |= (vhi >>  5) & 0x00001000; // 17 -> 12
        vi1    |= (vhi <<  2) & 0x00100000; // 18 -> 20
        vi1    |= (vhi <<  9) & 0x10000000; // 19 -> 28
...
static __device__ __forceinline__ float vec_dot_q5_0_q8_1(
    ...
    const int vh = get_int_b2(bq5_0->qh, 0) >> (4 * iqs);
