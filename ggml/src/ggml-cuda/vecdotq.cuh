...
template <int vdr> static __device__ __forceinline__ float vec_dot_q5_0_q8_1_impl(
    const int * vl, const int qh, const int * u, const float & d5, const half2 & ds8) {

    int sumi = 0;

#pragma unroll
    for (int i = 0; i < vdr; ++i) {
        const int qhi = qh >> (4 * i);
        // Scatter four packed qh bits into bit 4 of each byte with one multiply.
        const int qh0 = (qhi & 0x0F) * 0x02040810;
        const int vi0 = (vl[i] & 0x0F0F0F0F) | (qh0 & 0x10101010);
        sumi = ggml_cuda_dp4a(vi0, u[2*i+0], sumi); // SIMD dot product of quantized values

        const int qh1 = ((qhi >> 16) & 0x0F) * 0x02040810;
        const int vi1 = ((vl[i] >> 4) & 0x0F0F0F0F) | (qh1 & 0x10101010);
...
static __device__ __forceinline__ float vec_dot_q5_0_q8_1(
    ...
    const int qh = get_int_b2(bq5_0->qh, 0) >> (4 * iqs);
...
#pragma unroll
    ...
    return vec_dot_q5_0_q8_1_impl<VDR_Q5_0_Q8_1_MMVQ>(vl, qh, u, bq5_0->d, bq8_1->ds);
}
