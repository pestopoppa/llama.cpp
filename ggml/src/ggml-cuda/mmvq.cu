...
static constexpr __host__ __device__ int calc_nwarps(ggml_type type, int ncols_dst, mmvq_parameter_table_id table_id) {
    if (table_id == MMVQ_PARAMETERS_GENERIC) {
        ...
        // CDNA2 batch-1 Q6_K experiment: keep one physical wave per output-row block.
        // This removes the cross-wave reduction while preserving one block per output row.
        if (table_id == MMVQ_PARAMETERS_CDNA2 && ncols_dst == 1 && type == GGML_TYPE_Q6_K) {
            return 1;
        }
        switch (ncols_dst) {
            case 1:
