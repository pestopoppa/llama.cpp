...
void ggml_gemv_q8_0_8x8_q8_0(int                        n,
                             float * GGML_RESTRICT      s,
                             size_t                     bs,
                             const void * GGML_RESTRICT vx,
                             const void * GGML_RESTRICT vy,
...
void ggml_gemm_q8_0_8x8_q8_0(int                        n,
                             float * GGML_RESTRICT      s,
                             size_t                     bs,
                             const void * GGML_RESTRICT vx,
                             const void * GGML_RESTRICT vy,
...
static const ggml::cpu::tensor_traits * ggml_repack_get_optimal_repack_type(const struct ggml_tensor * cur) {
    ...
    if (cur->type == GGML_TYPE_Q4_0) {
        ...
        static const bool q8_0_8x8_on = []() {
            const char * s = getenv("GGML_REPACK_Q8_0_8X8");
            return s && atoi(s) != 0;
        }();
        if (q8_0_8x8_on && ggml_cpu_has_avx2()) {
            if (cur->ne[1] % 8 == 0) {
