...
struct DequantizerQ4K_AVX2 final : public BaseDequantizer<block_q4_K> {
    ...
    static constexpr bool pair_rows = true;
    Q4Bits_AVX2 bits;
};
...
struct DequantizerQ5K_AVX2 final : public BaseDequantizer<block_q5_K> {
    ...
    static constexpr bool pair_rows = false;
#else
...
static void mul_mat_qX_K_q8_2_X4_T(int n, const void * vx, size_t bx, const DataInfo& info, int nrc_x) {
        ...
        for (; ix + 1 < nrc_x; ix += 2) {
            ...
            for (int i = 0; i < nb; ++i) {
                ...
                for (int j = 0; j < QK_K/128; ++j) {
                    ...
                    __m512i values[4];
                    for (int k = 0; k < 4; ++k) {
                        values[k] = _mm512_inserti32x8(_mm512_castsi256_si512(deq.bits.values[k]),
                            deq2.bits.values[k], 1);
                    }
                    ...
                    for (int iy = 0; iy < nrc_y; ++iy) {
                        ...
                        auto sumi3 = _mm512_dpbusd_epi32(_mm512_setzero_si512(), values[2], yq[2]);
                        auto sumi4 = _mm512_dpbusd_epi32(_mm512_setzero_si512(), values[3], yq[3]);
    ...
    for (int ix = ix_start; ix < nrc_x; ++ix) {
        ...
        if constexpr (nrc_y == 1) {
            ...
            auto prepare_block = [&](int i, BlockData& data) {
                ...
                auto dy_h = _mm256_extractf128_ps(dy, 1);
                data.scales[0] = _mm256_mul_ps(scales_l, _mm256_set_m128(dy_l, dy_l));
                data.scales[1] = _mm256_mul_ps(_mm256_set_m128(scales_h, scales_h), _mm256_set_m128(dy_h, dy_h));
            };
            ...
            auto compute_sums = [&](int i, __m256i * sums) {
                ...
                for (int j = 0; j < QK_K/128; ++j) {
                    ...
                    sums[j] = _mm256_add_epi32(_mm256_unpacklo_epi64(sumi1, sumi3), _mm256_unpackhi_epi64(sumi1, sumi3));
#else
                    ...
                    sums[j] = _mm256_madd_epi16(_mm256_set1_epi16(1), sumi1);
#endif
                }
            };

            auto accumulate_block = [&](const BlockData& data, const __m256i * sums) {
                accd[0] = _mm256_fmadd_ps(data.my, data.mins, accd[0]);
                for (int j = 0; j < QK_K/128; ++j) {
                    accd[0] = _mm256_fmadd_ps(data.scales[j], _mm256_cvtepi32_ps(sums[j]), accd[0]);
                }
            };
            ...
            for (; i + 1 < nb; i += 2) {
                BlockData data[2];
                __m256i sums[2][QK_K/128];
                compute_sums(i + 0, sums[0]);
                prepare_block(i + 0, data[0]);
                compute_sums(i + 1, sums[1]);
                accumulate_block(data[0], sums[0]);
                prepare_block(i + 1, data[1]);
                accumulate_block(data[1], sums[1]);
            }
            if (i < nb) {
                BlockData data;
                __m256i sums[QK_K/128];
                compute_sums(i, sums);
                prepare_block(i, data);
                accumulate_block(data, sums);
            }
        } else {
        for (int i = 0; i < nb; ++i) {
...
#ifdef HAVE_FANCY_SIMD
            if constexpr (std::is_same_v<Dequantizer, DequantizerQ4K_AVX2>) {
                const __m256i q4_0 = _mm256_loadu_si256((const __m256i *)deq.x[i].qs + 0);
                const __m256i q4_1 = _mm256_loadu_si256((const __m256i *)deq.x[i].qs + 1);
                const __m256i q4_2 = _mm256_loadu_si256((const __m256i *)deq.x[i].qs + 2);
                const __m256i q4_3 = _mm256_loadu_si256((const __m256i *)deq.x[i].qs + 3);
                const __m512i ml = _mm512_set1_epi8(0xf);
                __m512i qx[4];
                qx[0] = _mm512_and_si512(_mm512_inserti32x8(_mm512_castsi256_si512(q4_0), q4_2, 1), ml);
                qx[1] = _mm512_and_si512(_mm512_srli_epi16(
                            _mm512_inserti32x8(_mm512_castsi256_si512(q4_0), q4_2, 1), 4), ml);
                qx[2] = _mm512_and_si512(_mm512_inserti32x8(_mm512_castsi256_si512(q4_1), q4_3, 1), ml);
                qx[3] = _mm512_and_si512(_mm512_srli_epi16(
                            _mm512_inserti32x8(_mm512_castsi256_si512(q4_1), q4_3, 1), 4), ml);

                for (int iy = 0; iy < nrc_y; ++iy) {
                    const block_q8_2_x4& y0 = q8.y[iy][2*i+0];
                    const block_q8_2_x4& y1 = q8.y[iy][2*i+1];
                    __m512i qy[4];
                    for (int k = 0; k < 4; ++k) {
                        qy[k] = _mm512_inserti32x8(
                            _mm512_castsi256_si512(_mm256_loadu_si256((const __m256i *)y0.qs + k)),
                    ...
                    auto dy4 = _mm_loadu_ps(d8 + 8*iy + 0);
                    auto d4d8 = _mm256_mul_ps(scales[0], _mm256_set_m128(dy4, dy4));
                    accd[iy] = _mm256_fmadd_ps(d4d8, _mm256_cvtepi32_ps(_mm512_castsi512_si256(sumi1)), accd[iy]);
                    dy4 = _mm_loadu_ps(d8 + 8*iy + 4);
                    d4d8 = _mm256_mul_ps(scales[1], _mm256_set_m128(dy4, dy4));
                    accd[iy] = _mm256_fmadd_ps(d4d8, _mm256_cvtepi32_ps(_mm512_extracti32x8_epi32(sumi1, 1)), accd[iy]);
                }
            } else
#endif
            for (int j = 0; j < QK_K/128; ++j) {
...
#ifdef HAVE_FANCY_SIMD
                const __m512i qx1 = _mm512_inserti32x8(_mm512_castsi256_si512(deq.bits.values[0]), deq.bits.values[0], 1);
                const __m512i qx2 = _mm512_inserti32x8(_mm512_castsi256_si512(deq.bits.values[1]), deq.bits.values[1], 1);
                const __m512i qx3 = _mm512_inserti32x8(_mm512_castsi256_si512(deq.bits.values[2]), deq.bits.values[2], 1);
                const __m512i qx4 = _mm512_inserti32x8(_mm512_castsi256_si512(deq.bits.values[3]), deq.bits.values[3], 1);

                int iy = 0;
                for (; iy + 1 < nrc_y; iy += 2) {
                    const block_q8_2_x4& y0 = q8.y[iy+0][2*i+j];
                    const block_q8_2_x4& y1 = q8.y[iy+1][2*i+j];
                    auto sumi1 = _mm512_dpbusd_epi32(_mm512_setzero_si512(), qx1,
                            _mm512_inserti32x8(_mm512_castsi256_si512(_mm256_loadu_si256((const __m256i*)y0.qs+0)), _mm256_loadu_si256((const __m256i*)y1.qs+0), 1));
                    auto sumi2 = _mm512_dpbusd_epi32(_mm512_setzero_si512(), qx2,
                            _mm512_inserti32x8(_mm512_castsi256_si512(_mm256_loadu_si256((const __m256i*)y0.qs+1)), _mm256_loadu_si256((const __m256i*)y1.qs+1), 1));
                    auto sumi3 = _mm512_dpbusd_epi32(_mm512_setzero_si512(), qx3,
                            _mm512_inserti32x8(_mm512_castsi256_si512(_mm256_loadu_si256((const __m256i*)y0.qs+2)), _mm256_loadu_si256((const __m256i*)y1.qs+2), 1));
                    auto sumi4 = _mm512_dpbusd_epi32(_mm512_setzero_si512(), qx4,
                            _mm512_inserti32x8(_mm512_castsi256_si512(_mm256_loadu_si256((const __m256i*)y0.qs+3)), _mm256_loadu_si256((const __m256i*)y1.qs+3), 1));
                    sumi1 = _mm512_add_epi32(_mm512_unpacklo_epi32(sumi1, sumi2), _mm512_unpackhi_epi32(sumi1, sumi2));
                    sumi3 = _mm512_add_epi32(_mm512_unpacklo_epi32(sumi3, sumi4), _mm512_unpackhi_epi32(sumi3, sumi4));
                    sumi1 = _mm512_add_epi32(_mm512_unpacklo_epi64(sumi1, sumi3), _mm512_unpackhi_epi64(sumi1, sumi3));
                    ...
                    accd[iy] = _mm256_fmadd_ps(d4d8, _mm256_cvtepi32_ps(_mm512_castsi512_si256(sumi1)), accd[iy]);
                    dy4 = _mm_loadu_ps(d8 + 8*(iy+1) + 4*j);
                    d4d8 = _mm256_mul_ps(scales[j], _mm256_set_m128(dy4, dy4));
                    accd[iy+1] = _mm256_fmadd_ps(d4d8, _mm256_cvtepi32_ps(_mm512_extracti32x8_epi32(sumi1, 1)), accd[iy+1]);
                }
                if (iy < nrc_y) {
                    const block_q8_2_x4& y = q8.y[iy][2*i+j];
                    auto sumi1 = _mm256_dpbusd_epi32(_mm256_setzero_si256(), deq.bits.values[0], _mm256_loadu_si256((const __m256i*)y.qs+0));
    ...
    for (int ix = 0; ix < nrc_x; ix += 16) {
        ...
        for (int ibl = 0; ibl < nbl; ++ibl) { // Block of 256
            ...
            for (int iy = 0; iy < nrc_y; ++iy) {
                ...
                acc[iy] = _mm512_fmadd_ps(m4,  _mm512_set1_ps(iqk_block_q8_K_sum(q8.y[iy][ibl])), acc[iy]); // iqk port: v6 block_q8_K has no .sum
                isum[iy] = _mm512_setzero_si512();
            }
        }
        for (int iy = 0; iy < nrc_y; ++iy) {
