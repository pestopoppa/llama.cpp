...
// iqk port: v6's refactored ggml-impl.h no longer pulls <immintrin.h> (ik's did),
// and iqk_common.h's own immintrin include further down is MSVC-only. Include it
// here for the non-MSVC x86 path so the AVX/AVX-512 intrinsics used below resolve.
#if !defined(_MSC_VER) && (defined(__x86_64__) || defined(__i386__) || defined(_M_X64))
#include <immintrin.h>
#endif
...
// iqk port: ik-specific enum values + block structs that v6's ggml.h/ggml-common.h
// lack (repacked _r*/_x4, IQ SOTA, q6_0). Referenced by the kernels as dead paths.
#include "iqk_ext_types.h"
#include "iqk_ext_blocks.h"
