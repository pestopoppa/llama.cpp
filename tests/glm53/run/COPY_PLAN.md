...
The runner resolves shared instruments through sibling `../runtime` and `../serve` directories. Do not duplicate them under `run/`. Do not copy generated plans, preflight JSON, runtime evidence, model manifests, checksums, bytecode caches, or binary fixtures. Runtime callers supply `--plan`, `--source-tree`, and `--manifest`.
