...
int main(int argc,char ** argv) {
    ...
    try {
        ...
        auto serial=make_ctx(p,m), batched=make_ctx(p,m), current_shape=make_ctx(p,m), rollback=make_ctx(p,m);
        decode(serial.get(),prompt,0,nv,false); decode(batched.get(),prompt,0,nv,false);
        decode(current_shape.get(),prompt,0,nv,false); decode(rollback.get(),prompt,0,nv,false);
        ...
        decode_sequential(current_shape.get(),n1,(llama_pos)prompt.size(),nv);
        auto current_rows=decode(current_shape.get(),b2,(llama_pos)prompt.size()+3,nv,true);
        const auto current_rb2=current_rows[2].v;
        ...
        o << "{\"schema\":\"epyc.glm53.real_divergence_diag.v1\",\"prompt_tokens\":" << prompt.size()
        ...
        emit_cmp(o,"batch2_reject_row_after_sequential_prefix_vs_serial",current_rb2,s2,tol,pass); o<<',';
        emit_cmp(o,"batch2_after_prior_rollback_vs_serial",rb2,s2,tol,pass);
        o << "},\"diagnostic_scope\":\"target-only exact accepted prefix; rejected IDs synthetic and must not affect retained state\",\"verdict\":\"" << (pass?"PASS":"FAIL") << "\"}\n";
        return pass?0:3;
