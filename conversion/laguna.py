...
class DFlashLagunaModel(DFlashModel):
    """Laguna DFlash drafter with the target architecture's decoder contract."""

    model_arch = gguf.MODEL_ARCH.DFLASH

    def set_gguf_parameters(self) -> None:
        # LagunaConfig injects MoE defaults into this dense drafter config.
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if match:
            ...
            n_aux = len(dflash_config.get("target_layer_ids", [])
