...
// ---- CPU15 Phase 2: anonymous-mmap'd expert-only NUMA copies ----
// When GGML_EXPERT_ANON_COPIES=1 (and CCD pools enabled), allocate one
// anonymous mmap per NUMA node sized to hold that node's share of MoE
// expert weights. mbind each region to its node BEFORE the memcpy so
// first-touch faults pages onto the target node. At compute time in
// ggml_compute_forward_mul_mat_id, the EP path looks up the local copy
// of each expert via ggml_ep_anon_lookup_().
//
// Memory budget: ~131.7 GiB of expert weights for REAP-246B distributed
...
struct ggml_ep_tensor_info {
    ...
    int    n_ccd_per_node;     // ceil(n_ccd / n_nodes)
    size_t per_expert_bytes;
    void * per_node_base[LLAMA_NUMA_REPLICA_MAX];   // per-node anon mmap base for THIS tensor's experts
    int    expert_to_node_idx[256];                 // [E]: index within owning node's expert array
};

#define LLAMA_EP_MAX_TENSORS 512
static int                       g_ep_anon_n_nodes = 0;
static int                       g_ep_anon_n_ccd = 0;
static int                       g_ep_anon_n_ccd_per_node = 0;
static int                       g_ep_anon_n_tensors = 0;
static ggml_ep_tensor_info       g_ep_anon_tensors[LLAMA_EP_MAX_TENSORS];
...
void llama_model_loader::init_mappings(bool prefetch, llama_mlocks * mlock_mmaps) {
    ...
    // CPU15 Phase 2: anonymous-mmap'd expert-only NUMA copies.
    // Stronger than Phase 1b's mbind on the file mmap (which doesn't reliably
    // move file-backed cached pages without CAP_SYS_NICE). For each MoE
    // expert tensor, this pass allocates a per-node anonymous mmap, mbind's
    // it to the target node, and copies expert bytes from the file mmap into
    // the appropriate per-node region. Pages are first-touched by the
    ...
    //
    // Memory: ~per_expert_bytes × n_experts total across nodes = roughly the
    // same bytes as the file mmap's expert region (just relocated to anon).
    // No 4× blowup like NUMA_REPLICATE (which copies the full model).
    if (use_mmap) {
        const char * env_anon = std::getenv("GGML_EXPERT_ANON_COPIES");
        if (env_anon && env_anon[0] && env_anon[0] != '0') {
            int n_nodes = 0;
            if (DIR * d = opendir("/sys/devices/system/node")) {
                struct dirent * ent;
                while ((ent = readdir(d))) {
            ...
            if (n_nodes < 2 || n_nodes > LLAMA_NUMA_REPLICA_MAX) {
                LLAMA_LOG_INFO("expert-anon-copies: %d NUMA node(s) — skipping (need 2..%d)\n",
                               n_nodes, LLAMA_NUMA_REPLICA_MAX);
            } else {
                const int n_ccd_per_node = (n_ccd + n_nodes - 1) / n_nodes;
                g_ep_anon_n_nodes = n_nodes;
                g_ep_anon_n_ccd = n_ccd;
                g_ep_anon_n_ccd_per_node = n_ccd_per_node;

                // Pass 1: enumerate expert tensors + tally per-node bytes
                ...
                std::vector<expert_tensor> exp_tensors;
                exp_tensors.reserve(weights_map.size() / 4);

                size_t per_node_bytes[LLAMA_NUMA_REPLICA_MAX] = {0};
                ...
                for (const auto & it : weights_map) {
                    ...
                    if (n_experts < n_ccd) continue;
                    if (n_experts > 256) {
                        LLAMA_LOG_WARN("expert-anon-copies: tensor '%s' has %lld experts > max 256, skipping\n",
                                       name.c_str(), (long long)n_experts);
                        continue;
                    }
                    const size_t total_bytes = ggml_nbytes(t);
                    const size_t per_expert_bytes = total_bytes / (size_t)n_experts;
                    if (per_expert_bytes * (size_t)n_experts != total_bytes) continue;

                    void * file_base = (char *)mappings[w.idx]->addr() + w.offs;
                    exp_tensors.push_back({file_base, n_experts, per_expert_bytes, name});

                    // Tally per-node bytes
                    for (int64_t e = 0; e < n_experts; ++e) {
                        const int ccd  = (int)(e % n_ccd);
                        const int node = ccd / n_ccd_per_node;
                        per_node_bytes[node] += per_expert_bytes;
                    }
                }

                if (exp_tensors.empty()) {
                    LLAMA_LOG_INFO("expert-anon-copies: no MoE expert tensors found, skipping\n");
                } else if (exp_tensors.size() > LLAMA_EP_MAX_TENSORS) {
                    LLAMA_LOG_WARN("expert-anon-copies: %zu expert tensors > LLAMA_EP_MAX_TENSORS=%d, skipping\n",
                                   exp_tensors.size(), LLAMA_EP_MAX_TENSORS);
                } else {
                    LLAMA_LOG_INFO("expert-anon-copies: allocating per-node anon mmaps for %zu expert tensors\n",
                                   exp_tensors.size());
                    for (int n = 0; n < n_nodes; ++n) {
                        LLAMA_LOG_INFO("expert-anon-copies:  node %d wants %.1f GiB\n",
                                       n, per_node_bytes[n] / (1024.0*1024.0*1024.0));
                    }

                    // Pass 2: allocate per-node anonymous mmaps + mbind to node
                    void * node_bases[LLAMA_NUMA_REPLICA_MAX] = {nullptr};
                    bool ok = true;
                    for (int n = 0; n < n_nodes; ++n) {
                        if (per_node_bytes[n] == 0) continue;
                        void * p = mmap(nullptr, per_node_bytes[n], PROT_READ | PROT_WRITE,
                                         MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
                        if (p == MAP_FAILED) {
                            LLAMA_LOG_WARN("expert-anon-copies: mmap for node %d (%zu bytes) failed: %s\n",
                                           n, per_node_bytes[n], strerror(errno));
                            ok = false;
                            break;
                        }
                        const unsigned long maxnode = 64UL;
                        unsigned long mask = (1UL << n);
                        long rc = syscall(SYS_mbind, p, per_node_bytes[n], MPOL_BIND, &mask, maxnode, 0);
                        if (rc != 0) {
                            LLAMA_LOG_WARN("expert-anon-copies: mbind node %d failed: %s\n", n, strerror(errno));
                        }
                        node_bases[n] = p;
                    }

                    if (!ok) {
                        // Roll back any allocations
                        for (int n = 0; n < n_nodes; ++n) {
                            if (node_bases[n]) munmap(node_bases[n], per_node_bytes[n]);
                        }
                        LLAMA_LOG_WARN("expert-anon-copies: allocation failed, falling back to baseline\n");
                    } else {
                        // Pass 3: register tensors + memcpy expert bytes
                        size_t cursor[LLAMA_NUMA_REPLICA_MAX] = {0};
                        for (size_t ti = 0; ti < exp_tensors.size(); ++ti) {
                            const expert_tensor & et = exp_tensors[ti];
                            ggml_ep_tensor_info * info = &g_ep_anon_tensors[g_ep_anon_n_tensors];
                            info->file_base = et.file_base;
                            info->n_experts = (int)et.n_experts;
                            info->n_nodes = n_nodes;
                            info->n_ccd = n_ccd;
                            info->n_ccd_per_node = n_ccd_per_node;
                            info->per_expert_bytes = et.per_expert_bytes;
                            for (int n = 0; n < LLAMA_NUMA_REPLICA_MAX; ++n) info->per_node_base[n] = nullptr;
                            for (int e = 0; e < 256; ++e) info->expert_to_node_idx[e] = -1;

                            // Track per-node within-tensor cursor
                            int per_tensor_node_count[LLAMA_NUMA_REPLICA_MAX] = {0};
                            // First, set per_node_base for this tensor: where in
                            // node N's region this tensor's experts begin.
                            for (int n = 0; n < n_nodes; ++n) {
                                if (node_bases[n]) {
                                    info->per_node_base[n] = (char *)node_bases[n] + cursor[n];
                                }
                            }

                            for (int64_t e = 0; e < et.n_experts; ++e) {
                                const int ccd  = (int)(e % n_ccd);
                                const int node = ccd / n_ccd_per_node;
                                info->expert_to_node_idx[(int)e] = per_tensor_node_count[node];
                                per_tensor_node_count[node]++;

                                void * src = (char *)et.file_base + (size_t)e * et.per_expert_bytes;
                                void * dst = (char *)node_bases[node] + cursor[node];
                                memcpy(dst, src, et.per_expert_bytes);
                                cursor[node] += et.per_expert_bytes;
                            }
                            g_ep_anon_n_tensors++;
                        }
                        size_t total_anon = 0;
                        for (int n = 0; n < n_nodes; ++n) total_anon += per_node_bytes[n];
                        LLAMA_LOG_INFO("expert-anon-copies: %d tensors / %.1f GiB total (%.1f GiB/node avg) ready\n",
                                       g_ep_anon_n_tensors, total_anon / (1024.0*1024.0*1024.0),
                                       total_anon / (1024.0*1024.0*1024.0) / (double)n_nodes);
                    }
                }
            }
        }
