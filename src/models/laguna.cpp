...
llama_model_laguna::graph::graph(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    for (int il = 0; il < n_layer; ++il) {
        // Residual-stream capture for speculative drafters.
    ...
    if (cparams.embeddings_nextn) {
        // DFlash target layer n_layer denotes the final block output, before
        // the model's output norm (HF hidden_states[n_layer]).
        cb(cur, "h_nextn", -1);
        res->t_h_nextn = cur;
    }
