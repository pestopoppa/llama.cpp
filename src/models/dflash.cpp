...
void llama_model_dflash::load_arch_tensors(llama_model_loader & ml) {
    ...
    if (decoder_laguna) {
        aux_norm = create_tensor(
                tn(LLM_TENSOR_ENC_AUX_NORM, "weight"),
                { n_embd, (int64_t) target_layer_ids.size() },
                0);
    }
    ...
    for (int i = 0; i < n_layer; ++i) {
        ...
        if (decoder_laguna) {
            ...
            const ggml_tensor * gate_meta = ml.get_tensor_meta(gate_name.str().c_str());
            if (gate_meta != nullptr) {
                const int64_t n_gate_out = gate_meta->ne[1];
                if (n_gate_out != n_head && n_gate_out != n_embd_head_k * n_head) {
                    GGML_ABORT(
                            "DFlash: unexpected attention gate width %lld at layer %d "
                            "(expected %lld per-head or %lld per-element)",
                            (long long) n_gate_out, i,
                            (long long) n_head,
                            (long long) (n_embd_head_k * n_head));
                }
                layer.wqkv_gate = create_tensor(gate_name, { n_embd, n_gate_out }, 0);
            }
        }
