...
static bool decode_or_fail(llama_context * ctx, llama_batch batch, const char * label) {
    ...
    if (ret != 0) {
        LOG_ERR("%s: %s decode failed, ret = %d\n", __func__, label, ret);
        return false;
    }
    return true;
}
...
int main(int argc, char ** argv) {
    ...
    if (!decode_or_fail(ctx_tgt, llama_batch_get_one( inp.data(), n_input - 1), "target prompt prefix") ||
        !decode_or_fail(ctx_tgt, llama_batch_get_one(&inp.back(),           1), "target prompt final token") ||
    ...
    while (true) {
...

        {
            ...
            if (!decode_or_fail(ctx_dft, batch_dft, "draft accepted target token")) {
                return 1;
            }
        ...
        for (int i = 0; i < n_draft; ++i) {
            ...
            if (!decode_or_fail(ctx_dft, batch_dft, "draft speculative batch")) {
                return 1;
            }
            ++n_past_cur;
            ++n_drafted;
        ...
        // evaluate the target model on the drafted tokens
        {
            ...
            if (!decode_or_fail(ctx_tgt, batch_tgt, "target speculative batch")) {
                return 1;
            }
            ++n_past_tgt;
        }
