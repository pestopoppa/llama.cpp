# PR Submission: CPU Paged Attention
...
This implementation is inspired by [PagedAttention](https://arxiv.org/abs/2309.06180) (Kwon et al., 2023) used in vLLM.
...
- [x] `test-kv-block` passes (19 tests)
- [x] `test-backend-ops -o FLASH_ATTN_EXT_PAGED` passes
- [x] Output determinism verified (same seed → same output)
- [x] Memory savings verified (up to 92% reduction)
- [x] Performance benchmarks across 3 model sizes
...
The authors used claude to construct schematics of vLLM's paged attention dependency, review our plan for integrating with existing flash attention rather than replacing it outright, build unit tests, review outstanding PRs pertaining to paged attention in llama.cpp repo, and draft PR submission.